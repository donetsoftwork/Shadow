...
public class GroupByMultiSelectTests
{
    ...
    public void Select()
    {
        ...
        var sql = _engine.Sql(select);
        Assert.Equal("SELECT t2.[UserId],MAX(t1.[Age]) AS MaxAge FROM [Users] AS t1 INNER JOIN [UserRoles] AS t2 ON t1.[Id]=t2.[UserId] GROUP BY t2.[UserId]", sql);
    }
    [Fact]
    public void Select2()
    {
        ...
        var sql = _engine.Sql(select);
        Assert.Equal("SELECT t2.[UserId],MAX([Age]) AS Age FROM [Users] AS t1 INNER JOIN [UserRoles] AS t2 ON t1.[Id]=t2.[UserId] GROUP BY t2.[UserId]", sql);
    }
}
